...
class SerialReader(threading.Thread):
    def __init__(self, port: str):
        ...
        self.port = port
        self.samples: queue.Queue[TelemetrySample] = queue.Queue(maxsize=500)
...
def main() -> None:
    ...
    log.writerow(["t_ms", "ax", "ay", "az", "gx", "gy", "gz",
                  "state", "rep_count", "quality", "elbow_angle",
                  "trunk_compensation", "cnn_prediction"])

    capture = cv2.VideoCapture(args.camera)
    camera_available = capture.isOpened() and mp is not None
    if camera_available and MP_LEGACY:
        ...
        pose = None
    baseline = None
    elbow_angles: deque[float] = deque(maxlen=100)
    ...
    try:
        while True:
            ...
            elbow_angle = None
            trunk_flag = False
            if camera_available and pose is not None:
                if MP_LEGACY:
                ...
                if results.pose_landmarks:
                    shoulder = point(landmarks, LANDMARK["LEFT_SHOULDER"])
                    elbow = point(landmarks, LANDMARK["LEFT_ELBOW"])
                    wrist = point(landmarks, LANDMARK["LEFT_WRIST"])
                    hip = point(landmarks, LANDMARK["LEFT_HIP"])
                    if shoulder and elbow and wrist:
                        elbow_angle = angle_degrees(shoulder, elbow, wrist)
                        if elbow_angle is not None:
                            elbow_angles.append(elbow_angle)
                    if shoulder and hip:
                        current = {"shoulder": shoulder, "hip": hip}
                        if baseline is None:
                            baseline = current
                        trunk_flag = trunk_compensation(baseline, current)

            elbow_range = ((max(elbow_angles) - min(elbow_angles))
                           if len(elbow_angles) >= 2 else None)
            state = latest.state if latest else "idle"
            quality = latest.quality if latest else "none"
            fusion = fuse_feedback(state, quality, elbow_range, trunk_flag,
                                   camera_available)
            reps = latest.rep_count if latest else 0
            overlay = [
                f"Mode: {fusion.mode}", f"Exercise: {state}", f"Repetitions: {reps}",
                f"Feedback: {fusion.feedback}", f"Serial: {reader.status}",
                f"CNN: {cnn_prediction or 'rule fallback'}",
            ]
            for i, text in enumerate(overlay):
                cv2.putText(frame, text, (15, 30 + i * 28),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.62, (30, 240, 255), 2, cv2.LINE_AA)

            if latest is not None and latest.timestamp_ms != last_logged_timestamp:
                log.writerow([latest.timestamp_ms, *latest.accel_g, *latest.gyro_dps,
                              latest.state, latest.rep_count, latest.quality,
                              elbow_angle, trunk_flag, cnn_prediction])
                log_file.flush()
                last_logged_timestamp = latest.timestamp_ms
